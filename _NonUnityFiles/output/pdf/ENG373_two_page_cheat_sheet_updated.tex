...
% Ultra-compact two-sided A4 layout. Four columns preserve scanability while
% the shared topic palette makes related boxes easy to find at a glance.
\geometry{left=0.20in,right=0.20in,top=0.17in,bottom=0.17in}
\setlength{\columnsep}{0.075in}
\setlength{\columnseprule}{0.12pt}
\def\columnseprulecolor{\color{lightblue!62!black}}
...
\pagestyle{empty}
\setlength{\abovedisplayskip}{0.65pt}
\setlength{\belowdisplayskip}{0.65pt}
...
\newcommand{\topicdot}[1]{\tikz[baseline=-0.48ex]\fill[#1] (0,0) circle (0.64ex);}
...
\begin{tcolorbox}[
  enhanced,
  colback=lightblue!4!white,
  colframe=lightblue!76!black,
  colbacktitle=lightblue!49!white,
  boxrule=0.25pt,
  boxsep=0.25pt,
  left=0.75pt,right=0.75pt,top=0.55pt,bottom=0.55pt,
  toptitle=0.35pt,bottomtitle=0.25pt,
  arc=0.45pt,outer arc=0.45pt,
  title={\fontsize{4.85pt}{5.05pt}\selectfont\textbf{#1}},
  fonttitle=\bfseries,
  coltitle=black,
  before skip=0.55pt,after skip=0.55pt,
  fontupper=\fontsize{4.15pt}{4.52pt}\selectfont\RaggedRight
]
#2
\end{tcolorbox}}
...
\fontsize{4.25pt}{4.62pt}\selectfont

\begin{center}
{\fontsize{8.3pt}{8.6pt}\selectfont\textbf{ENG373 Dynamic Systems \& Control --- Two-Page Exam Sheet}}\\[-1.5pt]
{\fontsize{4.7pt}{5pt}\selectfont Classical control, state space, estimation, optimisation and modern methods}
\end{center}
\vspace{-4.5pt}
\begin{center}
{\fontsize{4.55pt}{4.8pt}\selectfont
...
\frow{S(s)=\frac1{1+L(s)}}{sensitivity; disturbance rejection.}
\frow{T(s)=\frac{L(s)}{1+L(s)}}{complementary sensitivity; reference tracking.}
\frow{S+T=1}{fundamental robustness trade-off.}
...
\[
\begin{aligned}
\frac{\Omega}{V}&\approx\frac{K}{\tau s+1},\\[-1pt]
K&=\frac{K_t}{Rb+K_tK_b},\qquad
...
\frow{V>0,\ \dot V<0}{Lyapunov direct method: asymptotically stable.}
...
\[
\begin{aligned}
b_1&=\frac{a_{n-1}a_{n-2}-a_na_{n-3}}{a_{n-1}},\\[-1pt]
...
\tinyhead{Bandwidth:} $\omega_B=$ frequency where the magnitude has dropped by $3$ dB from the low-frequency value. Larger $\omega_B\Rightarrow$ faster closed-loop response.

\tinyhead{Resonance:} For a standard second-order system, a resonance peak occurs near $\omega\approx\omega_n$ when $\zeta<0.707$.
...
Advantage: Encodes both magnitude AND phase in a single curve.
\tinyhead{Nyquist low-frequency start:}
Type 0 $\rightarrow$ finite DC gain;\quad
Type 1 $\rightarrow -j\infty$;\quad
Type 2 $\rightarrow -\infty$ on the real axis.
...
\tinyhead{Delay tolerance:}
\[
t_d\approx\frac{\mathrm{PM}\pi}{180\,\omega_{gc}}
\]

Extra delay of approximately $t_d$ reduces the phase margin to zero.
...
\mybox{State Space \& Modern Control Map \Sdot\ \Odot\ \Ldot}{
\textbf{Navigate:} state models/tests $\to$ feedback/observers $\to$ LQR/Kalman/LQG $\to$ MPC, adaptive, learning, planning and safety. Use the coloured dots to connect definitions, workflows and comparison answers.
}
...
\[
\begin{aligned}
A&=\mat{0&1\\0&0},\quad B=\mat{0\\1},\\[-1pt]
...
\[
\begin{aligned}
A&\leftrightarrow A^T,\quad B\leftrightarrow C^T,\\[-1pt]
Q_{LQR}&\leftrightarrow GQ_wG^T,\quad R_{LQR}\leftrightarrow R_v,
\end{aligned}
\]
\[
...
\[
\begin{aligned}
\|W_mT\|_\infty&<1 &&\text{(robust stability)},\\[-1pt]
\|W_sS\|_\infty&<1 &&\text{(performance)}.
\end{aligned}
\]
...
\[
\begin{aligned}
J&=\sum_{i=0}^{N-1}\left(e_i^TQe_i+u_i^TRu_i\right)+e_N^TP_fe_N,\\[-1pt]
e_i&=x_i-r_i.
\end{aligned}
\]
...
\]
MIT rule; Lyapunov laws give stronger guarantees.
...
\[
\begin{aligned}
Q(s,a)\leftarrow{}&Q(s,a)+\alpha\bigl[r+\gamma\max_{a'}Q(s',a')\\[-1pt]
&{}-Q(s,a)\bigr].
\end{aligned}
\]
