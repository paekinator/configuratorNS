...
public class BlocksPanelUI : MonoBehaviour
{
    ...
    /// <summary>
    /// Fit the gallery's column count to its live viewport: six at the 16:9
    /// reference layout, fewer in a narrow browser window, so cards are never
    /// clipped behind the mask. The baked ScrollRect keeps horizontal
    /// scrolling as the last resort for any remaining overflow.
    /// </summary>
    void FitGalleryColumns()
    {
        var grid = gallery.GetComponent<GridLayoutGroup>();
        var scroll = gallery.GetComponentInParent<ScrollRect>();
        if (grid == null || scroll == null || scroll.viewport == null ||
            grid.constraint != GridLayoutGroup.Constraint.FixedColumnCount)
            return;

        float available = scroll.viewport.rect.width - grid.padding.horizontal;
        float pitch = grid.cellSize.x + grid.spacing.x;
        if (pitch <= 0f || available <= 0f)
            return;

        int columns = Mathf.Max(1, Mathf.FloorToInt((available + grid.spacing.x) / pitch));
