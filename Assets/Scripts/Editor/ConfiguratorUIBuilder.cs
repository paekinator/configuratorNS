...
public static class ConfiguratorUIBuilder
{
    ...
    static RectTransform BlockGallery(RectTransform page)
    {
        ...
        scroll.viewport = viewport;
        scroll.content = content;
        // BlocksPanelUI refits the column count to the live viewport (six at
        // the 16:9 reference, fewer in a narrow browser window), so the grid
        // normally never overflows sideways; horizontal stays enabled as the
        // safety net that keeps any overflow reachable instead of clipped.
