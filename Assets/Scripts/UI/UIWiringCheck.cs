...
public static class UIWiringCheck
{
    ...
    /// <summary>
    /// Rows WorkflowReviewUI clones onto the rail at runtime, below the baked
    /// ones: the space-size dialog, the parts list, the guide and the quote.
    /// </summary>
    static readonly string[] WorkflowRailButtons =
    {
        "Btn_SpaceSize", "Btn_PartsPrices", "Btn_BuildGuide", "Btn_Quote",
    };
    ...
    public static int RunAll(out List<string> failures)
    {
        ...
        if (rail != null)
        {
            // Baked rows first, in order; then the workflow rows that
            // WorkflowReviewUI appends at runtime (Set space size, Parts &
            // prices, the guide, the quote), also in order. In edit mode only
            // the baked rows exist; in play mode all of them must.
            int expected = 0;
            int workflow = 0;
            foreach (Transform child in rail)
            {
                ...
                {
                    if (child.name != RailButtons[expected])
                        failures.Add("rail button " + expected + " is " + child.name
                                     + ", expected " + RailButtons[expected]);
                    expected++;
                }
                else if (workflow < WorkflowRailButtons.Length)
                {
                    if (child.name != WorkflowRailButtons[workflow])
                        failures.Add("workflow rail button " + workflow + " is " + child.name
                                     + ", expected " + WorkflowRailButtons[workflow]);
                    workflow++;
                }
                else
                {
            ...
            checks++;
            if (expected != RailButtons.Length)
                failures.Add("the rail holds " + expected + " baked buttons, expected "
                             + RailButtons.Length);
            checks++;
            if (Application.isPlaying && workflow != WorkflowRailButtons.Length)
                failures.Add("the rail holds " + workflow + " workflow buttons at runtime, expected "
                             + WorkflowRailButtons.Length + " (WorkflowReviewUI appends them)");
        }
